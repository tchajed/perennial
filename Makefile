...
ALL_VFILES := $(shell find $(SRC_DIRS) \
	...
	-not -path "external/diaframe/supplements/*" \
	-not -path "external/diaframe/diaframe/legacy/*" \
	-not -path "external/diaframe/tutorial/ex3_*" \
	-not -path "external/diaframe/tutorial/ex4_*" \
	-name "*.v")
