...
Definition staged_value_inuse2 k e E1' E1 mj mj_wp mj_ukeep Φ Φc P :=
  (∃ E2 mj_wp_init mj_ishare mj_ushare γsaved γfinished γstatus γprop γprop',
      ...
      saved_prop_own γprop (later_tok ∗ wpc0 NotStuck k mj_wp E1 e
                     (λ v : val Λ, (wpc_crash_modality E1 mj_wp P) ∗ (wpc_crash_modality E1 mj_wp Φc ∧ Φ v))
                     (Φc ∗ (wpc_crash_modality E1 mj_wp P))) ∗
...
Lemma wpc_staged_inv_aux k e E1' mj mj_wp mj_ukeep Φ Φc P :
  staged_value_inuse2 k e E1' ⊤ mj mj_wp mj_ukeep Φ Φc P -∗
...
Proof.
  ...
  rewrite /wpc_pre. iSplit; last first.
  {
    ...
    iDestruct ("Hwand" with "[$]") as "HPs".
    rewrite /wpc_crash_modality.
    replace (⊤ ∖ D' ∖ E2) with (⊤ ∖ (E2 ∪ D')) by set_solver.
    iSpecialize ("HPs" with "[$] [$]").
    iMod (fupd2_mask_subseteq _ ) as "Hclo''"; last (iMod "HPs").
    { set_solver. }
    { set_solver. }
    iModIntro.
    iApply (step_fupd2N_wand with "HPs"). iIntros "HPs".
  ...
  }
  {
    ...
    iMod (saved_prop_alloc
            (later_tok ∗ wpc0 NotStuck k mj_wp ⊤ e2
              (λ v : val Λ, wpc_crash_modality ⊤ mj_wp P ∗ (wpc_crash_modality ⊤ mj_wp Φc ∧ Φ v))
              (Φc ∗ (wpc_crash_modality ⊤ mj_wp P)))%I) as (γprop_stored') "#Hsaved1''".
    ...
    iEval (rewrite -Heq_mj) in "Hitok".
    iDestruct (pri_inv_tok_split with "[$Hitok]") as "(Hitok_ukeep&Hitok_ushare)".
    iMod ("Hclo" with "[Hown' Hstatus' H Hitok_ishare Hitok_ushare Hltok']").
    { iNext.
      iEval (rewrite staged_inv_inner_unfold).
      iExists _, _, _, _, _. iFrame "∗ #".
      iLeft.
      ...
      iApply (step_fupd2N_inner_wand with "Hwpc"); auto.
      iIntros "(Hg&$)".
      iFrame. iApply (global_state_interp_le with "[$]"); lia.
    }
    iDestruct ("Hg_inv_clo" with "Hg") as "Hg".
    iMod (later_tok_incr with "[$]") as "(Hg&Hltok)".
    iMod (global_state_interp_le with "Hg") as "$".
...
Lemma wpc_staged_inv_inuse2 k E1 e Φ Φc Qs P :
  to_val e = None →
  later_tok ∗
  staged_value ⊤ Qs P ∗
  ((∀ mj_wp, wpc_crash_modality E1 mj_wp Φc) ∧
   (Qs -∗ ∀ mj_wp, ⌜ (/2 < mj_wp)%Qp ⌝ → WPC e @ NotStuck; k; E1
                                 ...
                                 {{ Φc ∗ wpc_crash_modality ⊤ mj_wp P }}))
  ⊢ WPC e @ NotStuck; k; E1 {{ Φ }} {{ Φc }}.
Proof.
  iIntros (Hnval) "(Hltok2&Hstaged&Hwp)".
  ...
  destruct (to_val e2) eqn:Heq_val.
  {
    ...
    iMod ("H" with "[$] [$]") as "H".
      ...
      { iNext.
        ...
        iSplit.
        { iPureIntro. split_and!; auto; try naive_solver.
          - etransitivity; first eapply Qp_le_min_r. reflexivity.
          - etransitivity; first eapply Qp_le_min_l.
            etransitivity; first eapply Qp_le_min_l.
            eapply Qp_le_min_l. }
        iFrame.
  ...
  iEval (rewrite -Heq_mj) in "Hitok".
  iDestruct (pri_inv_tok_split with "[$Hitok]") as "(Hitok_ukeep&Hitok_ushare)".
  iMod ("Hclo" with "[Hown' Hstatus' H Hitok_ishare Hitok_ushare Hltok2]").
  { iNext.
    iEval (rewrite staged_inv_inner_unfold).
    iExists _, _, _, _, _. iFrame "∗ #".
    iLeft.
  ...
  iAssert (staged_value_inuse2 k e2 ⊤ ⊤ mj mj_wp mj_ukeep Φ Φc P) with "[-]" as "Hsv".
  {
    iExists _, _, mj_ishare, _, _, _, _, _. iExists _. iFrame "∗".
    iSplit; first eauto.
    iFrame "Hsaved1'' Hsaved2''".
